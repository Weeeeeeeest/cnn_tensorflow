...
from model_lightnet import Model as Model
...
class Main_train():
    ...
    def train(self):
        X = tf.placeholder(tf.float32, [None, None, None, cf.Channel])
        Y = tf.placeholder(tf.float32, [None, cf.Class_num])
        keep_prob = tf.placeholder(tf.float32)

        ## Load network model
        logits = Model(x=X, phase='Train')
        #logits = model.forward(X)
        #pred = tf.nn.softmax(train_model)

        ## Loss, Optimizer
        loss_opt = tf.reduce_mean(
        ...
        with tf.Session(config=config) as sess:
            ...
            for ite in range(1, cf.Iteration+1):
                ...
                trainL, trainA = sess.run([loss_opt, accuracy], feed_dict={X:x, Y:y, keep_prob:1.0})
                ...
                if ite % cf.Save_train_step == 0 or ite == 1 or ite == cf.Iteration:
                    ...
                    """
                    L, A = sess.run([loss_opt, accuracy],
